...
guess of a set of most probable object classes, we then identify the salient object regions from the predicted top-ranked labels using the feedback neural nets, and re-classify those regions. Below are the implementation details:
\begin{center}
  \fbox{
...
Two exemplar images from ImageNet 2014 validation dataset are shown in Figure~\ref{fig:reclassification_examples}. All these images are wrongly classified as {\em ``Italian greyhound"} and {\em ``sunglass"} respectively after feedforward. As after feedback, the network determines potential salient ROIs and ``reconsider'' these salient regions as input, and make correct classifications, e.g., {\em ``Ibizan hound"} and {\em ``mask"} with high confidence scores. These two examples thoroughly illustrate how ``looking and thinking twice'' improves classification tasks. 
