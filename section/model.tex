...
We can re-interpret ReLU and max-pooling layers as components for feature selection in the neural nets. During feedforward computation, the ReLU and max-pooling layers select those neuron signals that are either \emph{confident enough or locally maximal} to proceed the message passing. 
